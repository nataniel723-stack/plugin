(function() {
    ...
    // Добавляем CSS-стили для отображения сетки эпизодов
    if (!$('style#emby-plugin-styles').length) {
        $('head').append(`
            <style id="emby-plugin-styles">
                ...
                .emby-ep-time { position: absolute; bottom: 0.4em; right: 0.4em; background: rgba(0,0,0,0.7); padding: 0.2em 0.5em; border-radius: 0.3em; font-size: 0.85em; color: #ddd; }
                ...
                .emby-filter { display: flex; align-items: center; justify-content: flex-start; padding: 1.5em 2em 0 2em; }
    ...
    function EmbySeriesComponent(object) {
        ...
        this.renderEpisodes = function(episodes) {
            ...
            scroll.append(filterPanel[0]); // Передача DOM элемента
            ...
            if (episodes.length === 0) {
                ...
                episodes.forEach(episode => {
                    ...
                    grid.append(item[0]); // Передача DOM элемента
                });
            }

            scroll.append(grid[0]); // Передача DOM элемента
            this.start();
        };
    ...
    function renderSettings(body) {
        ...
        body.append(wrap[0]); // Передача DOM элемента
    }
