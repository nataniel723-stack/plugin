(function() {
    ...
    if (!$('style#emby-plugin-styles').length) {
        $('head').append(`
            <style id="emby-plugin-styles">
                ...
                .emby-episode-card { width: calc(25% - 1.125em); cursor: pointer; transition: transform 0.2s, background 0.2s; border-radius: 0.5em; padding: 0.5em; box-sizing: border-box; position: relative; }
                ...
                .emby-filter { display: flex; align-items: center; justify-content: flex-start; padding: 1.5em 2em 0 2em; gap: 1em; flex-wrap: wrap; }
    ...
    function playVideo(item) {
        ...
        // Пробуем прямой URL к файлу через API Emby
    ...
    function EmbySeriesComponent() {
            ...
            if (current_episodes.length === 0) {
                ...
                current_episodes.forEach((episode) => {
                    ...
                    item.on('hover:enter click', function() {
                        ...
                        // Запрашиваем ВСЕ эпизоды сезона из Emby
                        let query = `/Items?ParentId=${emby_series_id}&Season=${seasonNumber}&IncludeItemTypes=Episode&Fields=Id,Name,IndexNumber,SortName&SortBy=SortName&SortOrder=Ascending`;
                        
                        let net = new Lampa.Reguest();
                        net.silent(buildApiUrl(query), (data) => {
                            if (is_destroyed) return;
                            console.log('Emby episodes for season', seasonNumber, ':', data);
                            
                            if (data && data.Items && data.Items.length > 0) {
                                // Сортируем по IndexNumber и берем нужный
                                let sortedEpisodes = data.Items.sort((a, b) => (a.IndexNumber || 0) - (b.IndexNumber || 0));
                                console.log('Sorted episodes:', sortedEpisodes.map(e => ({index: e.IndexNumber, name: e.Name})));
                                
                                // Берем эпизод по индексу (epNumber - 1, так как массив с 0)
                                let embyEpisode = sortedEpisodes[epNumber - 1];
                                
                                if (embyEpisode) {
                                    console.log('Playing episode:', embyEpisode.Name, 'ID:', embyEpisode.Id);
                                    playVideo(embyEpisode);
                                } else {
                                    body.html('<div class="emby-empty">Эпизод не найден в Emby</div>');
                                    setupNavigation();
                                }
                            } else {
        ...
        this.destroy = function() { is_destroyed = true; };
    }
