        ...
        Lampa.Listener.follow('full', function (e) {
            if (e.type == 'complite') {
                ...
                if ($('.rate--kp', render).hasClass('hide') && !$('.wait_rating', render).length) {
                    ...
                    // Дополнительная страховка: повторная замена иконок после полной прорисовки
                    setTimeout(function() {
                        var currentRender = Lampa.Activity.active().activity.render();
                        replaceRateIcons(currentRender);
                    }, 300);
                } else {
                    // Если рейтинг уже был показан (например, из кэша) — просто заменяем иконки
                    replaceRateIcons(render);
                }
            }
        });
