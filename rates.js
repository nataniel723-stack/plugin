(function () {
    ...
    function rating_kp_imdb(card) {
        ...
        function _showRating(data) {
            if (data) {
                ...
                // Заменяем текстовые метки на иконки
                replaceRateIcons(render);
            }
        }

        function replaceRateIcons(render) {
            function replaceInBlock(selector, icon) {
                var block = $(selector, render);
                if (!block.length) return;
                var valueDiv = block.find('> div').first();
                if (!valueDiv.length) return;
                // Удаляем все дочерние узлы, кроме valueDiv
                block.contents().each(function() {
                    if (this !== valueDiv[0]) {
                        $(this).remove();
                    }
                });
                // Добавляем иконку после значения
                block.append($('<span class="rate__icon"></span>').html(icon));
            }

            replaceInBlock('.rate--tmdb', rateIcons.tmdb);
            replaceInBlock('.rate--imdb', rateIcons.imdb);
            replaceInBlock('.rate--kp', rateIcons.kp);
        }
    }
    ...
    function startPlugin() {
        ...
        // Добавляем CSS для иконок
        var style = document.createElement('style');
        style.textContent = '.rate__icon svg { width: auto; height: 1em; fill: currentColor; vertical-align: middle; }';
