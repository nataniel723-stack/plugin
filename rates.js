	...
	function rating_kp_imdb(card) {
		...
		function _showRating(data) {
			if (data) {
				...
				replaceRatingIcons(render);
			}
		}
	}

	function replaceRatingIcons(render) {

		/* TMDB */
		var tmdbIcon = $('.rate--tmdb', render).find('> div:eq(1)');
		if (tmdbIcon.length && tmdbIcon.find('svg').length === 0) {
			tmdbIcon.html(
				'<svg width="45" height="45" viewBox="0 0 24 24" fill="currentColor">' +
				'<path d="M19.491 21.899c2.106 0 3.531-1.424 3.531-3.531V3.531C23.022 1.425 21.598 0 19.491 0H4.509C2.403 0 .978 1.424.978 3.531V24l1.809-2.101V3.531a1.721 1.721 0 0 1 1.719-1.719h14.982c.949 0 1.718.77 1.719 1.719v14.837a1.721 1.721 0 0 1-1.719 1.719H6.92l-1.81 1.812-.011-.014zM8.787 11.466H7.09v5.698h1.697c3.793 0 3.793-5.698 0-5.698zm0 4.559h-.551v-3.419h.551c2.215 0 2.215 3.418 0 3.418zM8.456 10.389h1.139V5.83h1.418V4.699H7.037V5.83h1.419v4.559zM14.063 7.201l-1.971-2.502h-.366v5.785h1.156v-3.18l1.182 1.531 1.183-1.531-.008 3.18h1.156V4.699h-.36l-1.971 2.502zM15.983 14.315c.358-.247.51-.689.526-1.124.023-1.004-.606-1.729-1.617-1.729h-2.255v5.706h2.255a1.695 1.695 0 0 1.681 1.694v-.02-.008c0-.466-.231-.878-.585-1.127l-.004-.003zm-2.204-1.714h1.013c.327 0 .526.255.526.573a.533.533 0 0 1-.526.574h-1.013V12.6zm1.013 3.427h-1.013v-1.139h1.027c.309 0 .559.25.559.559v.014a.566.566 0 0 1-.566.566h-.001zM936.9 938.29z"/>' +
				'</svg>'
			);
		}

		/* IMDb */
		var imdbIcon = $('.rate--imdb', render).find('> div:eq(1)');
		if (imdbIcon.length && imdbIcon.find('svg').length === 0) {
			imdbIcon.html(
				'<svg width="45" height="45" viewBox="0 0 32 32" fill="currentColor">' +
				'<path d="M22.231 11.348v9.159h2.134l0.143-0.581c0.173 0.219 0.386 0.397 0.63 0.526l0.011 0.005c0.246 0.107 0.533 0.169 0.835 0.169h0.009c0.016 0.001 0.035 0.001 0.054 0.001 0.344 0 0.662-0.111 0.921-0.299l-0.005 0.003c0.246-0.166 0.43-0.407 0.52-0.691l0.003-0.009c0.071-0.327 0.112-0.702 0.112-1.087 0-0.048-0.001-0.096-0.002-0.144v0.007-2.572c0.001-0.055 0.002-0.121 0.002-0.186 0-0.315-0.016-0.626-0.048-0.932l0.003 0.038c-0.033-0.198-0.107-0.375-0.214-0.527l0.003 0.004c-0.136-0.181-0.315-0.323-0.523-0.411l-0.008-0.003c-0.23-0.091-0.496-0.144-0.775-0.144h-0.
		/* Кинопоиск */
		var kpIcon = $('.rate--kp', render).find('> div:eq(1)');
		if (kpIcon.length && kpIcon.find('svg').length === 0) {
			kpIcon.html(
				'<svg width="46" height="46" viewBox="160 160 820 820" fill="currentColor">' +
				'<path d="M936.9 938.29c1.14.95 3.23 1.49 3-.79-.05-41.67-.06-83.34-.03-125 .1-1.02-.09-1.97-.57-2.85-98.43-39.97-197.21-79.3-295.65-118.78-8.08-3.24-16.18-6.75-24.28-9.73-47.06-18.9-94.1-37.8-141.12-56.68.55-.68 1.28-.93 2.18-.77 152.53 10.15 305.02 20.47 457.54 30.71 1.34.07 1.92-.57 1.75-1.91 0-41.67.01-83.33.03-124.99-.07-.95.23-2.87-1.22-2.88-152.03 9.7-304.11 19.9-456.14 29.49-.55-.02-.97-.25-1.27-.69 36.11-14.47 72.2-28.98 108.26-43.54 29.9-11.89 60.22-24.09 90.12-36.13 10.64-3.71 21.37-9.15 32.24-12.6 55.54-22.54 111.13-44.95 166.75-67.23 2.59-.74 7.83-3.26 10.28-3.72 16.81-6.68 33.55-13.57 50.22-20.67.55-.57.79-1.25.73-2.05.01-41.98.01-83.96.01-125.95-.01-1.46-.68-1.86-2.02-1.19-47.01 25.3-94.28 50.29-141.24 75.72-24.39 12.9-48.69 25.91-72.98 39.04-98.89 52.77-197.32 105.6-296.23 158.49-.41.24-.8.21-1.14-.1 1.13-1.52 2.37-2.97 3.72-4.36 87.18-89.45 174.31-178.94 261.4-268.47.84-1.01.58-1.47-.75-1.4-48 .01-96 .01-143.99 0-1.1-.13-2.13.04-3.11.5-59.72 82.11-118.86 164.5-178.07 246.94-1.47 1.55-3.15 5.07-4.93 5.99-.14-83.39-.17-166.78-.09-250.19-.01-1.33.14-3.5-1.84-3.24-38.65.02-77.31.03-115.96.01-1.09-.14-2.08.08-2.96.67-.26 232.18-.35 464.36-.27 696.56-.01 1.96-.25 3.4 2.26 3.2 38.97.01 77.95.02 116.93.03 1.98.26 1.82-1.91 1.82-3.23-.01-82.67-.02-165.35-.04-248.02-.09-.46.01-.85.3-1.19 1.19.62 2.12 1.53 2.78 2.73 59.91 83.16 119.94 166.23 180.07 249.22.98.45 2.01.62 3.11.49 48-.01 95.99 0 143.99.04 1.34.05 1.59-.42.76-1.43-7.45-7.58-14.87-15.19-22.26-22.83-79.05-80.56-157.98-161.2-236.8-241.91.29-.36.66-.43 1.1-.22 26.57 14.01 53.1 28.1 79.59 42.27 23.44 11.61 46.35 24.38 69.66 36.28 3.13 2.18 6.3 3.7 9.83 5.19 95.16 49.6 189.79 100.75 285.2 149.86 19.56 10.6 39.52 20.74 59.33 31.31z"/>' +
				'</svg>'
			);
		}
	}
