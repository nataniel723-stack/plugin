function () {
	 ...
	 *  ВАЖНО:
	 *  - сам .rate НЕ меняем
	 *  - display/flex/align-self НЕ меняем
	 *  - SVG находится внутри штатного второго div
	 *  - размер SVG задаётся только самим SVG
	 */


	function getTmdbIcon() {
		return '<svg xmlns="http://www.w3.org/2000/svg" width="1.25em" height="1.25em" viewBox="0 0 100 100" fill="none" style="display:block;width:1.25em;height:1.25em;max-width:1.25em;max-height:1.25em;">' +
			'<path d="M17 25h66v50H17z" stroke="currentColor" stroke-width="8" rx="8"/>' +
			'<path d="M32 65V35h8l10 16 10-16h8v30h-8V48L50 63 40 48v17h-8Z" fill="currentColor"/>' +
		'</svg>';
	}


	function getImdbIcon() {
		return '<svg xmlns="http://www.w3.org/2000/svg" width="1.25em" height="1.25em" viewBox="0 0 100 100" fill="none" style="display:block;width:1.25em;height:1.25em;max-width:1.25em;max-height:1.25em;">' +
			'<rect x="8" y="18" width="84" height="64" rx="8" fill="currentColor"/>' +
			'<path d="M25 34h7v32h-7V34Zm13 0h9c5 0 8 2 8 7v18c0 5-3 7-8 7h-9V34Zm7 26h1c2 0 3-1 3-3V43c0-2-1-3-3-3h-1v20Zm15-26h7l2 18 2-18h7l3 32h-7l-1-19-2 19h-5l-2-19-1 19h-7l4-32Z" fill="#000"/>' +
		'</svg>';
	}
	...
	function getKpIcon() {
		return '<svg xmlns="http://www.w3.org/2000/svg" width="1.25em" height="1.25em" viewBox="0 0 100 100" fill="none" style="display:block;width:1.25em;height:1.25em;max-width:1.25em;max-height:1.25em;">' +
			'<path d="M50 8v18M50 74v18M8 50h18M74 50h18M20 20l13 13M67 67l13 13M80 20 67 33M33 67 20 80" stroke="currentColor" stroke-width="7" stroke-linecap="round"/>' +
			'<path d="M36 25v50h8V54l17 21h11L53 50l18-25H60L44 46V25h-8Z" fill="currentColor"/>' +
		'</svg>';
	}


	function setRatingIcon(element, icon) {
		if (!element || !element.length) return;

		/*
		 * Меняем ТОЛЬКО содержимое штатного второго div.
		 * Сам .rate и его родительская структура остаются нетронутыми.
		 */
		element.html(icon);
	}


	function setRatingIcons(render) {
		if (!render || !render.length) return;

		setRatingIcon(
			$('.rate--tmdb', render).find('> div').eq(1),
			getTmdbIcon()
		);

		setRatingIcon(
			$('.rate--imdb', render).find('> div').eq(1),
			getImdbIcon()
		);

		setRatingIcon(
			$('.rate--kp', render).find('> div').eq(1),
			getKpIcon()
		);
	}
					...
					if (orig) {
						var _tmp = cards.filter(function (elem) {
							return containsTitle(
								elem.orig_title || elem.nameOriginal,
								orig
							) ||
							containsTitle(
								elem.en_title || elem.nameEn,
								orig
							) ||
							containsTitle(
	...
	function startPlugin() {
		...
		Lampa.Listener.follow('full', function (e) {
			...
			if (e.type == 'complite') {
				...
				/*
				 * Сначала просто заменяем штатные надписи
				 * TMDB / IMDB / KP на SVG.
				 *
				 * Никаких изменений родительских .rate.
				 */
				setRatingIcons(render);
