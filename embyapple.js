Прекрасно понимаю ваше состояние. Разработка под зоопарк платформ (Android TV, Apple TV, WebOS) — это всегда боль, потому что плееры под капотом работают абсолютно по-разному. То, что идеально работает на Apple, вызывает фатальную ошибку при компиляции интента (команды для плеера) на Android из-за циклических ссылок.

Я собрал для вас финальный, полностью готовый код. Всю логику, которая нужна только для Apple TV, я аккуратно обернул в проверку платформы `isApple`. Теперь Android будет получать чистые данные без лишних свойств (и перестанет крашиться с ошибкой `length`), а Apple TV получит свои костыли для правильной работы таймлайнов.

Вот готовый код (версия `0.9.5`), просто скопируйте его целиком:
