(function() {
    ...
    function EmbySeriesComponent(object) {
        ...
        this.create = function() {
            ...
            // Если не нашли, пробуем из object (если передали)
            if (!emby_series_id && object && object.emby_id) {
                emby_series_id = object.emby_id;
            }
            if (!tmdb_id && object && object.tmdb_id) {
        ...
        function renderEpisodes() {
            ...
            if (current_episodes.length === 0) {
                ...
                current_episodes.forEach(function(episode, index) {
                    ...
                    var time = ''; // длительность не используем
                    var quality = '';
    ...
    function handleEmbyClick(movie) {
        ...
        findInEmby(movie, function(item) {
            ...
            if (item.Type === 'Series') {
                ...
                // Сохраняем в глобальную переменную (как в первой версии)
                window.embySeriesData = {
                    emby_id: item.Id,
                    tmdb_id: tmdbId,
                    title: item.Name
                };
                // Передаём объект с movie для Explorer
                var params = {
                    emby_id: item.Id,
                    tmdb_id: tmdbId,
                    movie: movie
